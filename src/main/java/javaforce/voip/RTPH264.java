    ...
    while (len > 0) {
      ...
      if (packetLength > mtu) {
        ...
        while (packetLength > nalLength) {
          ...
          javaforce.voip.RTPChannel.buildHeader(packet, id, seqnum++, timestamp, ssrc, false);
          packet[12] = 28;  //FU-A
          packet[12] |= nri;
          packet[13] = type;
          if (first) {
        ...
        packet = new byte[12 + 2 + nalLength];
        javaforce.voip.RTPChannel.buildHeader(packet, id, seqnum++, timestamp, ssrc, len == nalLength);
        ...
        javaforce.voip.RTPChannel.buildHeader(packet, id, seqnum++, timestamp, ssrc, len == packetLength);
  ...
  public byte[] decode(byte rtp[]) {
    ...
    if (type >= 1 && type <= 23) {
      ...
      if ((rtp[13] & 0x80) == 0x80) {
        ...
        lastseqnum = javaforce.voip.RTPChannel.getseqnum(rtp, 0);
      } else {
        int thisseqnum = javaforce.voip.RTPChannel.getseqnum(rtp, 0);
