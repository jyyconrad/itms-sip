...
public class RTPChannel {
  ...
  protected RTPChannel(javaforce.voip.RTP rtp, int ssrc, SDP.Stream stream) {
    this.rtp = rtp;
    this.ssrc_src = ssrc;
    this.stream = stream;
  }
  ...
  public void writeDTMF(char digit, boolean end) {
    ...
    buildHeader(data, javaforce.voip.RTP.CODEC_RFC2833.id, getseqnum(), gettimestamp(160), getssrc(), false);
    switch (digit) {
      case '*':
        data[12] = 10;
        break;
  ...
  public boolean start() {
    ...
    if ((stream.codecs != null) && (stream.codecs.length > 0)) {
      Codec codec_rfc2833 = stream.getCodec(javaforce.voip.RTP.CODEC_RFC2833);
      if (codec_rfc2833 != null) {
        rfc2833_id = codec_rfc2833.id;
      }
      Codec codec_vp8 = stream.getCodec(javaforce.voip.RTP.CODEC_VP8);
      if (codec_vp8 != null) {
        vp8_id = codec_vp8.id;
      }

      Codec codec_h264 = stream.getCodec(javaforce.voip.RTP.CODEC_H264);
      if (codec_h264 != null) {
        h264_id = codec_h264.id;
      }

      Codec codec_h263_1998 = stream.getCodec(javaforce.voip.RTP.CODEC_H263_1998);
      if (codec_h263_1998 != null) {
        h263_1998_id = codec_h263_1998.id;
      }

      Codec codec_h263_2000 = stream.getCodec(javaforce.voip.RTP.CODEC_H263_2000);
      ...
      if (stream.type == SDP.Type.audio) {
        ...
        for(int a=0;a<stream.codecs.length;a++) {
          ...
          if (codec.equals(javaforce.voip.RTP.CODEC_G711u)) {
            coder = coder_g711u;
            JFLog.log("codec = g711u");
            break;
          } else if (codec.equals(javaforce.voip.RTP.CODEC_G711a)) {
            coder = coder_g711a;
            JFLog.log("codec = g711a");
            break;
          } else if (codec.equals(javaforce.voip.RTP.CODEC_GSM)) {
            coder = coder_gsm;
            JFLog.log("codec = gsm");
            break;
          } else if (codec.equals(javaforce.voip.RTP.CODEC_G722)) {
            coder = coder_g722;
            JFLog.log("codec = g722");
            break;
          } else if (codec.equals(javaforce.voip.RTP.CODEC_G729a)) {
            coder = coder_g729a;
        ...
        for(int a=0;a<stream.codecs.length;a++) {
          Codec codec = stream.codecs[a];
          if (codec.equals(javaforce.voip.RTP.CODEC_H263)) {
            JFLog.log("codec = H.263");
            break;
          } else if (codec.equals(javaforce.voip.RTP.CODEC_H263_1998)) {
            JFLog.log("codec = H.263-1998");
            break;
          } else if (codec.equals(javaforce.voip.RTP.CODEC_H263_2000)) {
            JFLog.log("codec = H.263-2000");
            break;
          } else if (codec.equals(javaforce.voip.RTP.CODEC_JPEG)) {
            JFLog.log("codec = JPEG");
            break;
          } else if (codec.equals(javaforce.voip.RTP.CODEC_H264)) {
            JFLog.log("codec = H.264");
            break;
          } else if (codec.equals(javaforce.voip.RTP.CODEC_VP8)) {
            JFLog.log("codec = VP8");
            break;
          }
        }
  ...
  public boolean change(SDP.Stream new_stream) {
    ...
    stream = new_stream;
    if (stream.type == SDP.Type.audio) {
      if (new_stream.hasCodec(javaforce.voip.RTP.CODEC_G711u)) {
        coder = coder_g711u;
      } else if (new_stream.hasCodec(javaforce.voip.RTP.CODEC_G711a)) {
        coder = coder_g711a;
      } else if (new_stream.hasCodec(javaforce.voip.RTP.CODEC_GSM)) {
        coder = coder_gsm;
      } else if (new_stream.hasCodec(javaforce.voip.RTP.CODEC_G722)) {
        coder = coder_g722;
      } else if (new_stream.hasCodec(javaforce.voip.RTP.CODEC_G729a)) {
        coder = coder_g729a;
      }
      dtmf = new DTMF(coder.getSampleRate());
    }
  ...
  protected void processRTP(byte data[], int off, int len) {
    lastPacket = javaforce.voip.RTP.now;
    if (rtp.rawMode) {
