...
public class SIPTCPTransportServer implements SIPTransport {
  ...
  private ArrayList<javaforce.voip.SIP.Packet> packets = new ArrayList<javaforce.voip.SIP.Packet>();
...
//        if (packets.size() == 0) continue;
        javaforce.voip.SIP.Packet tmp = packets.remove(0);
    ...
    public void process() {
      ...
      while (active) {
        try {
          ...
          javaforce.voip.SIP.Packet packet = new javaforce.voip.SIP.Packet();
          packet.data = data;
